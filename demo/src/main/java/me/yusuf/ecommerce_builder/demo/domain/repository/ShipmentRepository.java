...
@org.springframework.stereotype.Repository("shipmentRepository")
...
public interface ShipmentRepository extends Repository<Shipment,Integer> {
    ...
    @PostAuthorize("returnObject.getOrder().getUser().getUsername() == principal.username or returnObject.getSeller().userId==T(me.yusuf.ecommerce_builder.shared.types.entity.User).cast(principal).id or hasRole('ROLE_ADMIN')")
    @Nullable Shipment findById(Integer id);
    @PostAuthorize("returnObject.content[#this.getSeller().userId == T(me.yusuf.ecommerce_builder.shared.types.entity.User).cast(principal).username] or returnObject.content[ #this.getOrder().getUser().getUsername() == T(me.yusuf.ecommerce_builder.shared.types.entity.User).cast(principal)?.username] or hasRole('ROLE_ADMIN')")
    ...
    @Query("SELECT s from Shipment s where s.seller.userId = ?#{T(me.yusuf.ecommerce_builder.shared.types.entity.User).cast(principal).id}")
    ...
    @PreAuthorize("T(me.yusuf.ecommerce_builder.shared.types.entity.User).cast(principal).id == #shipment.getSeller().userId or hasRole('ROLE_ADMIN')")
    <S extends Shipment> S save(@NonNull @Param("shipment") S _shipment);
    @PreAuthorize("T(me.yusuf.ecommerce_builder.shared.types.entity.User).cast(principal).id == shipment.getSeller().userId or hasRole('ROLE_ADMIN')")
    <S extends Shipment> void delete(S shipment);
    @PreAuthorize("shipmentRepository.findById(id).getSeller().userId == T(me.yusuf.ecommerce_builder.shared.types.entity.User).cast(principal).id or hasRole('ROLE_ADMIN')")
