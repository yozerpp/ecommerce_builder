package me.yusuf.ecommerce_builder.demo.domain.coupon;
...
public interface CouponRepository extends Repository<Coupon, String> {
    @PreAuthorize("isAuthenticated() and hasRole('ROLE_SELLER') and T(me.yusuf.ecommerce_builder.demo.domain.user.User).cast(principal).userId = #_coupon.seller.userId")
    <C extends Coupon> C save(@NonNull @P("_coupon") C coupon);
    @PreAuthorize("isAuthenticated() and (hasRole('ROLE_SELLER') and T(me.yusuf.ecommerce_builder.demo.domain.user.User).cast(principal).userId = #_coupon.seller.userId or hasRole('ROLE_ADMIN'))")
