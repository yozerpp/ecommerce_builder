...
import me.yusuf.ecommerce_builder.editor.tool.transpiler.ast.expression.AdditiveExpr;
import me.yusuf.ecommerce_builder.editor.tool.transpiler.ast.expression.ComparisonExpr;
import me.yusuf.ecommerce_builder.editor.tool.transpiler.ast.expression.EqualityExpr;
import me.yusuf.ecommerce_builder.editor.tool.transpiler.ast.expression.Expr;
import me.yusuf.ecommerce_builder.editor.tool.transpiler.ast.expression.LogicalAndExpr;
import me.yusuf.ecommerce_builder.editor.tool.transpiler.ast.expression.MultiplicativeExpr;
...
public class ExpressionTests extends TestBase {
    ...
    public void testPrimaryString(){
        ...
        var parser = getParser(in);
        var ctx = parser.değer();
        Primary.StringLiteral stringLiteral = (Primary.StringLiteral) visitor.visitDeğer(ctx);
        // The string literal includes the quotes as per the grammar.
        Assertions.assertEquals("\"hello\"", stringLiteral.string);
    }
    ...
    public void testUnaryNot(){
        ...
        var parser = getParser(in);
        var ctx = parser.tekliDenklem();
        PostfixExpr postfix = (PostfixExpr) visitor.visitTekliDenklem(ctx).operand();
        // Assert that the operator is "değil" using direct field access.
        Assertions.assertTrue(postfix.hasNot());
        var num = postfix.primary();
    ...
    public void testMultiplicativeExprSimpleMultiplication(){
        ...
        PostfixExpr left = leftUnary.operand();
