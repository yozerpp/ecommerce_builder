...
import me.yusuf.ecommerce_builder.editor.tool.transpiler.generated.TurkishPseudoCodeBaseVisitor;
...
public class ASTBuilderVisitor extends TurkishPseudoCodeBaseVisitor<ASTNode> {
    ...
    public PluginDef visitIşlevTanımı(TurkishPseudoCodeParser.IşlevTanımıContext ctx) {
        ...
        ret.setEntityName(ctx.varlıkİsmi().getText());
    ...
    @Override
    public ExpressionStatement visitDenklemİfadesi(TurkishPseudoCodeParser.DenklemİfadesiContext ctx) {
    ...
    public ForeachStatement visitHerBiriİfadesi(TurkishPseudoCodeParser.HerBiriİfadesiContext ctx) {
        ...
        ret.setCollection( visitDenklem(ctx.denklem()));
    ...
    public UnaryExpr visitTekliDenklem(TurkishPseudoCodeParser.TekliDenklemContext ctx) {
        return new UnaryExpr(ctx.eksiİşareti()!=null?ctx.eksiİşareti().getText():null,visitDeğilİfadesi(ctx.değilİfadesi()));
    }
    ...
    @Override
    public PostfixExpr visitDeğilİfadesi(TurkishPseudoCodeParser.DeğilİfadesiContext ctx) {
        return new PostfixExpr(visitDeğer(ctx.değer()),ctx.DEĞİL()!=null);
    }

    @Override
    public Primary.Identifier visitDeğişken(TurkishPseudoCodeParser.DeğişkenContext ctx) {
        if (ctx.ilkelDeğişken()!=null) return visitIlkelDeğişken(ctx.ilkelDeğişken());
        else {
            Primary.Identifier root= new Primary.Identifier(ctx.üyeErişimiDeğişken().İSİM(0).getText(),null);
            Primary.Identifier cur = root;
            for (int i = 1; i < ctx.üyeErişimiDeğişken().İSİM().size(); i++){
                cur.memberAccess= new Primary.Identifier();
                cur = cur.memberAccess;
                cur.identifier = ctx.üyeErişimiDeğişken().İSİM(i).getText();
            }
            return root;
        }
    }
    ...
    @Override
    public Primary visitSabitDeğer(TurkishPseudoCodeParser.SabitDeğerContext ctx) {
        if (ctx.SAYI() != null) {
            ...
            Primary.StringLiteral ret = new Primary.StringLiteral();
    ...
    @Override
    public Expression visitDeğer(TurkishPseudoCodeParser.DeğerContext ctx) {
